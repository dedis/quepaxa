...
const statusTimeout = 40              // time to wait for a status request in seconds
...
func New(name int64, cfg *configuration.InstanceConfig, logFilePath string, batchSize int64, testDuration int64, arrivalRate int64, requestType string, operationType int64, debugLevel int, debugOn bool, keyLen int, valLen int, slowdown string) *Client {
	cl := Client{
		name:                   name,
